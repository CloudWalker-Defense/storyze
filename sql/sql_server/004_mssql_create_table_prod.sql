...
PRINT 'Starting Production Table Creation for MSSQL...';
GO
...
PRINT 'Creating table [prod].[mssql_findings]...';
GO
...
BEGIN TRY
    ...
    PRINT 'Table [prod].[mssql_findings] created successfully with inline constraints.';

END TRY
BEGIN CATCH
    PRINT 'ERROR: Failed to create table [prod].[mssql_findings].'
...
-- Add Indexes
PRINT 'Creating indexes on [prod].[mssql_findings]...';
GO
...
END TRY
BEGIN CATCH
    PRINT 'ERROR: Failed to create indexes on [prod].[mssql_findings].'
    PRINT 'ErrorNumber: ' + CAST(ERROR_NUMBER() AS VARCHAR(10))
    PRINT 'ErrorSeverity: ' + CAST(ERROR_SEVERITY() AS VARCHAR(10))
    PRINT 'ErrorState: ' + CAST(ERROR_STATE() AS VARCHAR(10))
    PRINT 'ErrorProcedure: ' + ISNULL(ERROR_PROCEDURE(), 'N/A')
...
PRINT 'Production Table Creation Script Finished.';
GO